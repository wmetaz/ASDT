...
def compute_seg_label_refine(ori_img, cam_label, norm_cam, bg_up):
    ...
    bg_score = np.expand_dims(bg_up, axis=0)
...
def get_data_from_chunk_v2(chunk, args):
    ...
    img_original = np.zeros((len(chunk),dim, dim, 3)).astype(np.uint8)
    ...
    for i, piece in enumerate(chunk):
        ...
        img_ori = cv2.resize(img_temp, (dim, dim))
        img_original[i, :, :, :] = img_ori.astype(np.uint8)
    ...
    return images, ori_images, labels, croppings, img_original
